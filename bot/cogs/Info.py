...
class Info(commands.Cog):
    ...
    @commands.command(name="user_info", aliases=["user", "member"], description="Sends information about a target user's discord account")
    async def user_info_command(self, ctx, target: t.Optional[discord.Member]):
        target = target or ctx.author

        activity = None
        for _activity in target.activities:
            if (isinstance(_activity, discord.Game) or isinstance(_activity, discord.Streaming)
                    or isinstance(_activity, discord.Spotify) or _activity.type == discord.ActivityType.playing):
                activity = _activity
                break
